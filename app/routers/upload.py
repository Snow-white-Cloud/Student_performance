...
async def upload_csv_batch(file: UploadFile = File(...)): # Требуем файл
    ...
    count_students = 0 # Количество новых студентов
    ...
    try:
        async with get_connect() as connect:
            ...
            count_students += len(students_cache) # Количество студентов до вставок
            ...
            for row in reader:
                ...
                if key in students_cache:
                    batch_grades_know_stud.append((data_grade.grade, data_grade.date_of_mark, students_cache[key]))
                else:
                    batch_grades_unknow_stud.append((data_grade.full_name, data_grade.study_group, data_grade.grade, data_grade.date_of_mark))
            ...
            if batch_grades_unknow_stud:
                ...
                await flush_batch_grades_unknow_stud(connect, batch_grades_unknow_stud, students_cache)
            # Разница между количеством студентов в таблице до вставок и после == количество новых студентов
            count_students = len(students_cache) - count_students
...
async def flush_batch_grades_unknow_stud(connect, batch_grades_unknow_stud: list, students_cache: dict):
    ...
    logger.info(f"Начинаем загрузку {len(new_students)}")
    async with connect.transaction():    
        try:
            await connect.executemany(
                """
